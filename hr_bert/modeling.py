...
class AlbertModel(object):
    ...
    def __init__(self,
        ...
        Args:
          ...
          input_sememe_mask: (optional) int32 Tensor of shape [batch_size * seq_length, 1, sememe_length].
        ...
        input_sememe_shape = get_shape_list(input_sememe_ids, expected_rank=2)
        sememe_length = int(input_sememe_shape[1] / seq_length)
        ...
        with tf.variable_scope(scope, default_name="bert"):
            with tf.variable_scope("embeddings"):
                ...
                word_embedding_output_reshaped = tf.reshape(
                    self.word_embedding_output,
                    [batch_size * seq_length, config.embedding_size]
                )
                print("word_embedding_output_reshaped: ", word_embedding_output_reshaped.shape)
                # [batch_size * seq_length, 1, embedding_size]
                word_embedding_output_reshaped = tf.expand_dims(word_embedding_output_reshaped, axis=[1])
                print("word_embedding_output_reshaped: ", word_embedding_output_reshaped.shape)

                # embedding lookup on the sememes.
                # [batch_size, seq_length, embedding_size]
                # input_sememe_ids: [batch_size, seq_length. sememe_length]
                input_sememe_ids = tf.reshape(input_sememe_ids, [batch_size * seq_length, sememe_length])
                # [batch_size * seq_length, sememe_length, embedding_size]
                (self.sememe_embedding_output,
                 self.output_sememe_embedding_table) = embedding_lookup(
                    input_ids=input_sememe_ids,
                    vocab_size=config.sememe_vocab_size,
                    embedding_size=config.embedding_size,
                    initializer_range=config.initializer_range,
                    word_embedding_name="sememe_embeddings",
                    use_one_hot_embeddings=use_one_hot_embeddings)

                # word embedding attend to sememe embeddings
                # q: [batch_size * seq_length, 1, embedding_size],
                # k & v: [batch_size * seq_length, sememe_length, embedding_size]
                print("input_sememe_mask: ", input_sememe_mask.shape)
                with tf.variable_scope("attention_to_sememe"):
                    with tf.variable_scope("to_sememe"):
                        sememe_attention_output = attention_layer(
                            from_tensor=word_embedding_output_reshaped,
                            to_tensor=self.sememe_embedding_output,
                            attention_mask=input_sememe_mask,
                    ...
                    with tf.variable_scope("output"):
                        sememe_attention_output = dense_layer_3d_proj(
                            sememe_attention_output,
                            config.embedding_size,
                            int(config.embedding_size/config.num_attention_heads),
                            create_initializer(config.initializer_range),
                            None,
                            name="dense")
                        sememe_attention_output = dropout(sememe_attention_output, config.hidden_dropout_prob)

                self.word_embedding_output = self.word_embedding_output + \
                                             tf.reshape(sememe_attention_output, [batch_size, seq_length, config.embedding_size])
